/**
 * SketchGrid - 4-sketch display with selection
...
interface SketchGridProps {
  ...
  onSelectSketch: (sketch: GeneratedImage) => void;
  isLoading: boolean;
}

const SketchGrid: React.FC<SketchGridProps> = ({
  sketches,
  selectedSketch,
  onSelectSketch,
  isLoading,
      ...
      <div className="p-4 bg-slate-900/60 rounded-lg border border-slate-800/50">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="aspect-[9/10] rounded-lg bg-slate-800/50 border border-slate-700/50 animate-pulse flex items-center justify-center"
            >
