...
function ContextMenu({ x, y, onClose, onAdd }: ContextMenuProps) {
  ...
  return (
    <motion.div
      ...
      className="fixed z-50 py-1 bg-slate-900 border border-slate-700/60 rounded-lg shadow-xl min-w-[180px]"
      style={{ top: y, left: x }}
      ...
      actions={
        isDirty ? (
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-2 py-0.5 rounded text-[10px] font-medium bg-amber-600/20 text-amber-300 hover:bg-amber-600/30 transition-colors disabled:opacity-50"
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        ) : undefined
